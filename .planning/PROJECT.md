...
**Target features:**
- Entidad **Parcela** con `nombre` y `codigo` únicos por plantación + `descripcion` opcional (max 10.000 chars)
- Constraint de unicidad de **Grupo** cambia: `nombre` y `codigo` únicos **por parcela** (antes: por plantación) — prerequisito de la migración
- **SubID de árbol** cambia a formato `ParcelaCode + GroupCode + SpeciesCode + Position` (incluye Parcela para evitar colisiones cuando el mismo código de grupo aparece en distintas parcelas)
...
- **Migración de data Supabase**: 32 → 3 plantaciones (1 producción + 2 prueba consolidadas), 11 plantaciones eliminadas, 6.321 árboles reales preservados bajo "San Sebastián de la Selva", **6.776 SubIDs re-computados** con el nuevo formato
...
- **(v1.1) Campos extra de Parcela** (foto, color, área) — solo `nombre`, `codigo` y `descripcion` (text opcional, max 10.000 chars) en este milestone
...
| (v1.1) Grupos: unicidad de `nombre`/`codigo` cambia de per-plantación a per-parcela | Prerequisito técnico de la migración: 17 plantaciones se unifican bajo "San Sebastián de la Selva" y muchas tienen códigos de grupo coincidentes (L1, L2...); per-parcela uniqueness lo resuelve naturalmente y refleja mejor el modelo mental del técnico (códigos repetidos por parcela son OK) | — Pending |
| (v1.1) SubID prepende código de Parcela | Consecuencia directa del cambio de unicidad: dos grupos en distintas parcelas pueden compartir código, así que SubID necesita el componente Parcela para mantener unicidad. Re-cálculo batch para los 6.776 árboles preservados durante la migración | — Pending |
| (v1.1) `descripcion` de Parcela limitada a 10.000 caracteres | Permite notas extensas (ubicación detallada, características del terreno) sin abrir la puerta a payloads gigantes que rompan sync; CHECK constraint en DB y validación UI con contador | — Pending |
