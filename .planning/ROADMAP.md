...
| 12. Photo Storage + Sync | 0/3 | Planned |  |
...
Plans:
- [ ] 12-01-PLAN.md — Drizzle migration (fotoSynced column), schema update, install expo-image-manipulator, PhotoService resize at capture, TreeRepository photo sync helpers, Supabase Storage RLS migration
- [ ] 12-02-PLAN.md — SyncService photo upload/download functions, useSync hook extension (incluirFotos params, photo progress states), unit tests
- [ ] 12-03-PLAN.md — UI layer: PlantationDetailHeader two-button layout (Descargar/Subir) with CheckboxRow toggles, SyncProgressModal photo states, TreeRow Ionicons with sync dot, PlantationDetailScreen wiring, visual checkpoint
