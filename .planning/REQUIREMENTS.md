...
- [ ] **MIGR-04**: **Cluster A** unificado bajo plantación nueva `"San Sebastián de la Selva"` (Otoño 2026) con 17 parcelas siguiendo el mapping: SSS-Loma-P1..P13 → Loma-P1..P13 (códigos LP1..LP13), SSS-Medio-P1..P4 → Medio-P1..P4 (códigos MP1..MP4); todos los grupos y árboles preservados (**233 grupos, 7.124 árboles** — números actualizados 2026-05-25 tras reemplazo Pp↔SSS-Medio-P4 via 012c)
- [ ] **MIGR-05**: **Cluster B** unificado bajo plantación nueva `"Pruebas - SSS"` (Otoño 2026) con 2 parcelas: "Selva Original" (código SO, fuente plantación actual `San Sebastián de la Selva` 67bfd338-...) y "P3 Vieja" (código P3V, fuente plantación actual `SSS - P3` be0eb2d5-...); 5 grupos, 114 árboles preservados
- [ ] **MIGR-06**: **Cluster C** unificado bajo plantación nueva `"Pruebas - La Morita"` (Primavera 2026) con 2 parcelas: "La Morita" (código LM, fuente `Misiones - La Morita` 5c064556-...) y "Zona 1" (código Z1, fuente `La Morita Zona 1` 7ab85e48-...); 5 grupos, 341 árboles preservados
- [ ] **MIGR-07**: **16 plantaciones eliminadas** (cascade delete: plantación + grupos + árboles) — lista expandida en auditoría 2026-05-25:
  - **Originales (11):** `00000000` La Maluka, `e072775e` SSS-LOMA-P1 vacío, `80b85acd` Plantación Abril, `26e190db` Hfhj, `747981d3` Plantacion test 1, `0eea0006` Aa, `a536bd66` Plant 2, `09a315e2` Plant 3, `203beee5` Plant 4, `6d2e80b0` Plantación test 2, `7fea8850` Plant Test 4
  - **Nuevas (5):** `0dfa2ced` Ghh, `58b43096` Achalay, `461a37c7` Reserva Achalay, `ad648fab` prueba (ahora ya mismito), `5ccc718b` prueba (ahora)
  - Totalizando aproximadamente 47 grupos y 461 árboles eliminados
- [ ] **MIGR-08**: Normalización de estados: 269 grupos con `estado='sincronizada'` en server → `estado='finalizada'` (alineado con Phase 13)
- [ ] **MIGR-09**: **Re-cálculo de SubIDs** para los **7.579 árboles** preservados (Cluster A 7.124 + B 114 + C 341) con el nuevo formato `ParcelaCode + GroupCode + SpeciesCode + Position` (PARC-08). Operación batch dentro de la misma transacción que la consolidación; verificar que ningún SubID quede `NULL` y que todos sean únicos dentro de su plantación
- [ ] **MIGR-10**: Verificación post-migración: conteos esperados (3 plantaciones consolidadas, 21 parcelas, **243 grupos, 7.579 árboles** — más 1 plantación deprecated standalone con 2 grupos/117 árboles que queda viva para cleanup deferred), referencias FK consistentes, **nombres y códigos de Grupo únicos por parcela** (no por plantación), nombres y códigos de Parcela únicos por plantación, SubIDs únicos dentro de cada plantación, queries cargan correctamente
