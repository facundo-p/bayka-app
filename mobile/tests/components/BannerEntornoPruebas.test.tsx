...
jest.mock('../../src/config/entorno', () => ({ ES_ENTORNO_DE_PRUEBAS: true, VERSION_APP: 'v1.0.0 (3)' }));
...
describe('BannerEntornoPruebas', () => {
  ...
  it('en la app TEST muestra el aviso con la versión', () => {
    const { getByText } = render(<BannerEntornoPruebas />);
    expect(getByText('ENTORNO DE PRUEBAS · v1.0.0 (3)')).toBeTruthy();
  });
