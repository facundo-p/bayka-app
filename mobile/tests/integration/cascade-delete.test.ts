 ...
 * Tests: plantation + groups + trees + plantation_species + plantation_users all removed
...
describe('Cascade delete', () => {
  test('deletePlantationLocally removes plantation, all groups, and all trees', async () => {
    ...
    const sg1 = createTestGroup({ plantacionId: plantation.id, codigo: 'L01', nombre: 'Linea 1' });
    const sg2 = createTestGroup({ plantacionId: plantation.id, codigo: 'L02', nombre: 'Linea 2' });
    await db.insert(groups).values(sg1);
    await db.insert(groups).values(sg2);

    for (let i = 1; i <= 3; i++) {
      await db.insert(trees).values(createTestTree({ groupId: sg1.id, especieId: sp.id, posicion: i, subId: `L01EUC${i}` }));
      await db.insert(trees).values(createTestTree({ groupId: sg2.id, especieId: sp.id, posicion: i, subId: `L02EUC${i}` }));
    }
    ...
    const sgRows = await db.select().from(groups).where(eq(groups.plantacionId, plantation.id));
  ...
  test('delete of plantation with 0 groups succeeds without error', async () => {
    const plantation = createTestPlantation();
    await db.insert(plantations).values(plantation);
