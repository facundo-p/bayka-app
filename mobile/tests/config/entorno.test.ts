...
describe('formatearEtiquetaBuild', () => {
  it('versión + commit: el commit identifica el build que se está probando', () => {
    expect(formatearEtiquetaBuild('1.0.0', 'a1b2c3d')).toBe('v1.0.0 · a1b2c3d');
  });

  it('sin commit degrada a solo la versión', () => {
    expect(formatearEtiquetaBuild('1.0.0')).toBe('v1.0.0');
    expect(formatearEtiquetaBuild('1.0.0', '')).toBe('v1.0.0');
  });

  it('conserva el sufijo -dirty del commit', () => {
    expect(formatearEtiquetaBuild('1.0.0', 'a1b2c3d-dirty')).toBe('v1.0.0 · a1b2c3d-dirty');
  });
});
